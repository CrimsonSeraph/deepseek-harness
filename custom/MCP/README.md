...
├── README.md                  # 本文件（总览）
├── game-engine/               # 分类：游戏引擎
│   └── godot-mcp/             #   Godot 4.x 引擎控制（157 工具）
│       ├── <完整可运行副本> + README.md + README.upstream.md
│       └── scripts/godot-mcp.ps1
└── ide/                       # 分类：IDE
...
| --- | --- | --- | --- | --- | --- | --- |
| [godot-mcp](game-engine/godot-mcp/README.md) | game-engine | 控制 Godot 引擎：项目管理、场景编辑、运行时操控（`game_eval` 等） | 157 | stdio | ✅ 已注册 `mcp__godot__*` | **复制**（原位置在运行中） |
...
| 工具 | 安装 |
| --- | --- |
| godot-mcp | 见 [其 README](game-engine/godot-mcp/README.md)：`git clone` + `npm install` + `npm run build`；需 Node ≥18、Godot ≥4.4（本机 Steam 版 4.7.x，`GODOT_PATH` 用户环境变量已配置） |
...
.\game-engine\godot-mcp\scripts\godot-mcp.ps1              # 校验（构建+Godot 路径+握手+工具数）
.\game-engine\godot-mcp\scripts\godot-mcp.ps1 -Serve       # 前台启动（stdio）
...
| 内容 | 原位置（继续使用） | 迁移副本/文档 |
| --- | --- | --- |
| godot-mcp 完整安装 | `<GODOT_MCP_HOME>` | `game-engine\godot-mcp\`（完整可运行副本） |
...
- **godot-mcp**：运行时 `game_*` 工具需在 Godot 项目注册 `McpInteractionServer` autoload
  （`build/scripts/mcp_interaction_server.gd`，监听 9090）；项目管理类工具无需。
- **qtcreator-mcp**：MCP 服务器存活于 IDE 进程内，**Qt Creator 需保持运行**；端口固定 46327。
- **切换生效副本**：若要把 DSH 的 godot 桥改指向本副本，编辑 `cordis.patch.yml` 中 `mcp-godot`
  的 `args` 路径（见子 README 注释行），HMR 自动重连。
